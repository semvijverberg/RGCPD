...
ERA_12d_RGCPD = {'ERA-5':(era5_12d_RGCPD_sm, [None, 'sst(CPPA)'])}
ERA_vs_PEP = {'ERA-5':(era5_1d_CPPA_lag0, ['sst(PEP)+sm', 'sst(PDO,ENSO)+sm', 'sst(CPPA)+sm'])}

datasets_path  = ERA_12d_RGCPD
...
for dataset, tuple_sett in datasets_path.items():
    ...
    for keys_d in keys_d_list:
        ...
        fc.fit_models(stat_model_l=stat_model_l, lead_max=lags_i, 
                           keys_d=keys_d, kwrgs_pp=kwrgs_pp)
        print(fc.keys_d)
...
RV_name = 't2mmax'
working_folder = f'/Users/semvijverberg/surfdrive/RGCPD_mcKinnon/{fc.hash}_forecast'
