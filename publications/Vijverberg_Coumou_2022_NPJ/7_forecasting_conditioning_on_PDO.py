...
This scripts generates (and plots) the results presented in Figure 7 and
SI-Table 1.
...
filepath_df_PDOs = os.path.join(data_dir, 'df_PDOs_monthly.h5')
...
#%% get timeseries
rg.list_import_ts = [(None, filepath_df_PDOs)]
rg.get_ts_prec()
...
lowpass = 0.5 # which low-pass filtered PDO timeseries to remove from normal SST
lwp_method = 'rm'
region_labels = [1, 2]
...
#%% removing PDO
df_prec = merge_lagged_wrapper(rg.df_data.copy() , [1,2], keys)
df_PDOs23 = merge_lagged_wrapper(rg.df_data.copy() , [2,3], ['PDO0.5rm'])
df_prec = rg.get_subdates_df(df_prec, years=(1980,2020))
df_PDOs23 = rg.get_subdates_df(df_PDOs23, years=(1980,2020))
df_prec_and_PDO = df_prec.merge(df_PDOs23, left_index=True, right_index=True)

lag_1_keys = ['1..1..sst_1', '1..2..sst_1']
df_prec_and_PDO[lag_1_keys], f = wPCMCI.df_data_remove_z(df_prec_and_PDO,
                                                         z_keys=['PDO0.5rm_2'],
                                                         lag_z=0,
                                                         keys=lag_1_keys,
                                                         standardize=False)

lag_2_keys = ['1..1..sst_2', '1..2..sst_2']
df_prec_and_PDO[lag_2_keys], f = wPCMCI.df_data_remove_z(df_prec_and_PDO,
                                                         z_keys=['PDO0.5rm_3'],
                                                         lag_z=0,
                                                         keys=lag_2_keys,
                                                         standardize=False)
df_prec_and_PDO = df_prec_and_PDO.merge(rg.df_splits.loc[pd.IndexSlice[:, dates], :],
                                        left_index=True, right_index=True)
out = prediction_wrapper(df_prec_and_PDO, lags=np.array([0]),
                         target_ts=target_ts_temp,
                         keys=lag_1_keys + lag_2_keys,
                         match_lag=False, n_boot=n_boot)

pred, df_test, df_test_m, df_boot = out[:4]
print('mean MAE-SS: {:.2f} - ({:.2f}-{:.2f})'.format(df_test_m.loc[0][0]['MAE'],
      df_boot.quantile(0.05/2).loc[0]['MAE'],
      df_boot.quantile(1-0.05/2).loc[0]['MAE']))
#%%
