...
Step for SI-Figure 2 to reproduce results of NPJ paper:
"The role of the Pacific Decadal Oscillation and ocean-atmosphere interactions in driving US temperature variability"

This script is used to generate the results that are plotted in SI-Figure 2.
It loads the data that is generated by clustering.py and creates the plot
shown in Figure 8
...
np_array_xy = np_array_xy[:,[1,0]] # oeps was wrong order, quick fix.

scatter = [['all', [[np_array_xy, {'s':size, 'zorder':1,
                                  'color':colors,
                                  'edgecolors':'black'}]] ]]

# for countour lines around the western and eastern US clusters
if region == 'USCA':
...
fg = plot_maps.plot_labels(xrclustered,
                      {'size':3,
                        'scatter':scatter,
                        'zoomregion':selbox,
                        'mask_xr':mask_cl,
...
fg.fig.set_facecolor('white')
fg.fig.axes[0].set_facecolor('white')
...
#%% Figure 8
# Without scatter points, with purple outline of clusters
