...
path_out_main = os.path.join(main_dir, 'publications/paper2/output/east_RW_sm/')
...
list_of_name_path = [(name_or_cluster_label, TVpathRW),
                     ...
                     ('sm1', os.path.join(path_raw, 'sm1_1979-2018_1_12_daily_1.0deg.nc'))]
...
list_for_MI   = [BivariateMI(name='z500', func=BivariateMI.corr_map,
                 ...
                 BivariateMI(name='sm1', func=BivariateMI.corr_map,
                              kwrgs_func={'alpha':.05, 'FDR_control':True},
...
save = True
units = 'Corr. Coeff. [-]'
subtitles = np.array([[f'{name2} vs eastern RW']])
rg.plot_maps_corr(var=name2, row_dim='split', col_dim='lag',
...
rg.plot_maps_corr(var='z500', row_dim='lag', col_dim='split',
 ...
 #%%
rg.list_for_MI = [BivariateMI(name='sm1', func=BivariateMI.corr_map,
                              kwrgs_func={'alpha':.05, 'FDR_control':True},
                              distance_eps=500, min_area_in_degrees2=5,
                              calc_ts='pattern cov', selbox=sst_green_bb)]

rg.calc_corr_maps(var=name2)
rg.cluster_list_MI(var=name2)
rg.quick_view_labels(var=name2, median=True)
rg.get_ts_prec(precur_aggr=1)
rg.store_df(append_str=f'RW_and_sm2_fb_tf{rg.tfreq}')
...
for f in freqs[:]:
    ...
    rg.df_data = rg.df_data.rename({'z5000..0..z500_sp':'Rossby wave (z500)', '0..0..sm2_sp':'N-Pacific SST',
                                    '15..0..Trop. Pac. SST_sp':'Trop. Pac. SST',
                                    '15..2..Trop. Pac. SST':'Nina',
                                    '0..0..sm1_sp':'SM1'}, axis=1)

    keys = [['Rossby wave (z500)','SM1']]
    k = keys[0]
...
for f in freqs:
    ...
    sst = functions_pp.get_df_test(rg.df_data, cols=k[1])
    sst_mask = (sst < sst.quantile(q=.75)).squeeze()
    new_mask = np.logical_and(sst_mask, new_mask)
    sumyears = new_mask.astype(int).groupby(new_mask.index.year).sum()
    sumyears = list(sumyears.index[sumyears >= sumyears.quantile(.90)])
    ...
    plt.savefig(os.path.join(rg.path_outsub1, f'subset_dates_tf{f}.pdf'))
