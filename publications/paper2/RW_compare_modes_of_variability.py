...
for west_east in ['west', 'east']:
    ...
    for eof, name, selbox in [(z500EOF, namez500, z500_green_bb), (v300EOF, namev300, v300_green_bb)]:
        ...
        subtitles = np.array([['{name} hPa - 2nd EOF loading pattern']])
    ...
    df_data = rg.df_data.copy().mean(axis=0,level=1) ; df_data.pop('2ts_x')
    ...
    if west_east == 'east':
        columns = ['E-RW', 'v300-EOF1','v300-EOF2', 'PNAliu', 'PNAcpc']
    elif west_east == 'west':
        columns = ['W-RW', 'v300-EOF1','v300-EOF2']

    df_ana.plot_ts_matric(df_c[columns], win=15)
    filename = os.path.join(rg.path_outsub1, f'cross_corr_{west_east}_15d.pdf')
