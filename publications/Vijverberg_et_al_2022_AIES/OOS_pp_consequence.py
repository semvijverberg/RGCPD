...
path_1_no_OOS = f'{experiment}/{standard_subfolder}/df_data_{fc_type}{btoos}_all_CD'
...
def plot_table(subtitle, path, ax):
    ...
    df_scores = [df_scores[i] for i in [0,2,1]]
    ...
    metrics_plot = ['BSS', 'accuracy', 'precision']
...
ax1 = plot_table('No OOS pre-processing', os.path.join(path_main, path_1_no_OOS), ax[0])
ax2 = plot_table('No OOS pre-processing', os.path.join(path_main, path_1_no_OOS), ax[1])
ax3 = plot_table('No OOS pre-processing', os.path.join(path_main, path_1_no_OOS), ax[2])