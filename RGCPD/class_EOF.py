...
class EOF:
    ...
    def get_pattern(self, filepath, df_splits=None, selbox=None, start_end_date=None,
        ...
        if df_splits is None:
            ...
            coords = [splits, [f'EOF{n+1}_'+self.name for n in range(self.neofs)],
                      self.ds_EOF.latitude.values, self.ds_EOF.longitude.values]
            self.eofs = xr.DataArray(data,
                                    coords=coords,
                                    dims = ['split', 'eof', 'latitude', 'longitude'])
    ...
    def get_ts(self, tfreq_ts=1):
        ...
        for s in splits:
            ...
            for i, e in enumerate(neofs):
                ...
                if i == neofs.size-1:
                    dfs = dfs.merge(self.df_splits.loc[s], left_index=True, right_index=True)
            df_data_s[s] = dfs
